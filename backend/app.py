...
# ─── FIX: Add CORS so Streamlit frontend can call this API ───────────────────
...
# ─── FIX: Frontend calls /student-list but this endpoint was missing ─────────
@app.get("/student-list")
def get_student_list():
    """Returns just first names for the dropdown in Streamlit frontend."""
    try:
        with engine.connect() as conn:
...
# ─── FIX: POST for /ask so long questions with special chars don't break ─────
@app.post("/ask")
def ask(body: dict):
...
"""
            ...
            return {"student": row.first_name, "cgpa": row.cgpa, "ai_response": answer}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def attendance_summary(student_id: int):
    try:
        with engine.connect() as conn:
            ...
            answer = ask_gemini(prompt)
            return {"attendance": row.attendance_percentage, "ai_response": answer}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def fee_status(student_id: int):
    try:
        with engine.connect() as conn:
            ...
            answer = ask_gemini(prompt)
            return {"pending_fee": float(row.pending_fee), "status": row.status, "ai_response": answer}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        with engine.connect() as conn:
            ...
            companies = conn.execute(
                text("SELECT company_name, min_cgpa FROM companies")
            ).fetchall()
...
CGPA: {student.cgpa}
Eligible Companies: {', '.join(eligible) if eligible else 'None currently'}
